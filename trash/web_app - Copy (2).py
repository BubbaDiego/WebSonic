...
from data.data_locker import Position
from data.data_locker import AssetType
import requests
import os
import uuid
import asyncio
...
def create_position():
    try:
        ...
        position = {
            ...
            "asset_type": asset,
            "position_type": position_type,  # Assign position type here
            "entry_price": 0.0,  # Default to 0.0 if not provided
            "liquidation_price": 0.0,  # Default to 0.0 if not provided
            "current_travel_percent": 0.0,
            "value": size * collateral,  # Example value calculation
            "collateral": collateral,
            "size": size,
            "wallet": "Default",
            "leverage": size / collateral if collateral else 1.0,
            "last_updated": datetime.now(),
            "current_price": 0.0,  # Default to 0.0 if not provided
            "liquidation_distance": 0.0,  # Default to 0.0 if not provided
        }
